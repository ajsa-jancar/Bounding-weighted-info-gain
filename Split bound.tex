...
Motivated by the regional decomposition ideas in sections \ref{sec: transobjective} and \ref{sec: indepregions}, we now return to the original sequential weighted information gain objective. Even without independence between regions, the same regional structure can still be exploited to derive an upper bound. The key observation is that posterior variance can only decrease when conditioning on additional observations. Consequently, the contribution from points inside a given region can be bounded by considering only the subsequence of points lying in that region.

This leads to a decomposition of the weighted objective into regional information gain terms with rescaled effective noise variances. Intuitively, larger weights correspond to smaller effective noise levels, and therefore to larger regional information gains.
