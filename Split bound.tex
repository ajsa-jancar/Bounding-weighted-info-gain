...
Second, one can define a regional objective by restricting the target
information gain to a subset $A\subseteq\mathcal X$. In this formulation,
the weighting is applied to the target region where uncertainty should be
reduced, rather than to the locations chosen for sampling. Consequently,
the objective remains a function of the sampled set and preserves the usual
submodularity properties, allowing existing transductive information gain
techniques to be applied (see Appendix~\ref{app:regional_objective}). However, this corresponds to a different notion of task relevance: the
region of interest is specified through the prediction target itself. 
