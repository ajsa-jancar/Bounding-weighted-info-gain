...
For ordinary information gain, the objective can be written as a submodular set function, and the corresponding marginal gains satisfy a diminishing returns property~\cite{Krause2008}. This structure underlies many of the theoretical guarantees in active learning and Bayesian experimental design, in particular allowing us to compare greedy selection objectives directly with maximum information gain.
...
This result reduces the analysis of weighted information gain to the
analysis of standard information gain on individual regions. Consequently,
existing information-gain bounds for Gaussian process kernels
~\cite{Seeger2008,Srinivas2009}
can be applied separately on each region, allowing the effects of geometry,
volume and intrinsic dimension to be captured explicitly. In particular, the resulting bounds reveal situations in which weighted exploration can be substantially more efficient than global exploration, especially when highly weighted regions occupy only a small subset of the domain or lie on lower-dimensional structures.

\begin{proposition*}[Informal consequence for Squared Exponential kernels]
For Squared Exponential kernels, applying the split bound reveals that the
largest improvements arise when highly weighted regions have lower effective dimension than the ambient domain since the dimension affects both the multiplicative constants and the exponent of the logarithmic information-gain rate. Reducing the size of the weighted region also improves the bound, but only through lower-order geometric constants, as regional volume enters
logarithmically.
\end{proposition*}
